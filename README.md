...
구글·네이버·Bing·유튜브에서 검색하면 **내 옵시디언 볼트의 설교·설교조각·묵상·성경구절 노트가 검색 결과 옆에 함께 뜹니다.**
...
│  구글/네이버/Bing/    │        │  Local REST API (본문/칩) │
│  유튜브 옆 표시       │        │                          │
│                     │        │  A4P Helper (설정 도우미)  │
└─────────────────────┘        └──────────────────────────┘
```
...
- **카테고리 필터** — `전체 | 설교 | 조각 | 묵상 | 성경 | 자료` 칩 + **건수 배지** (0건 칩은 흐림). 폴더 키워드는 설정에서 내 볼트 구조에 맞게 변경. 주석·강해 폴더의 노트는 결과를 도배하지 않도록 **기본 제외** (설정 '주석 노트 숨기기'에서 해제 가능).
- **다양성 정렬** — 성경구절 검색 시 결과를 설교→조각→묵상→자료 순으로 교차 배치해 한 종류가 상위를 독점하지 않음 (설정에서 끌 수 있음).
- **더 보기** — 기본 10건 이후 버튼 한 번으로 추가 표시 (재검색 없이 즉시).
- **설교 날짜 배지** — `260412_대_…` 형식의 설교 파일명을 인식해 `2026-04-12 · 대예배` 배지로 표시.
- **doctrine 칩 재검색** — 카드의 신학 주제 칩(✝️)을 클릭하면 그 주제로 바로 재검색.
- **결과 캐시** — 검색 결과를 5분간 기억해 뒤로가기로 돌아오면 즉시 표시 (백그라운드 재검색으로 갱신).
...
- **4개 검색엔진** — 구글·네이버·Bing·유튜브. 사이드바가 없는 사이트에서는 우측 플로팅 패널로 표시 (유튜브는 검색결과 페이지에서만).
- **에디토리얼 디자인** — 그림자·색 상자 없이 헤어라인과 타이포그래피 위계로 조판한 A4P 브랜드 그린 기본 스킨. 헤더 ◐ 버튼으로 라이트/다크 수동 전환 (기본은 OS 자동).
- 원본의 강력한 기능 유지 — 멀티볼트(최대 6개), 관련도 바, 본문 미리보기, 테마 9종, 카드 스킨 4종, 키보드 내비(j/k/Enter/y/c).
...
- 본 포크: **A4P (abadcsh)** — 목회자 특화 기능(성경구절 인식·카테고리·doctrine 칩·설정 코드 온보딩·네이버/Bing/유튜브 지원·에디토리얼 디자인·도우미 플러그인)
